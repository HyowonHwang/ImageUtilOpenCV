...
void cvShowInvDFT(IplImage* im, CvMat* dft_A, int dft_M,int dft_N, FILE *fp, char* src);
CvMat* cvShowDFT(IplImage* im, int dft_M, int dft_N, char* src);

void deblurring(const std::string filename)  {
  ...
  im1 = cvLoadImage(filename.c_str(),1 );
  ...
  //float
  //vals[]={.000625,.000625,.000625,.003125,.003125,.003125,.000625,.000625,.000625};
  ...
  //Perform inverse of kernel (check & comment out) - commented as it overwrites
  //dft_B
  ...
  CvMat complex_lmcMat; 
  cvGetMat(complex_ImC, &complex_lmcMat);
  // Perform Inverse
  cvShowInvDFT(im, &complex_lmcMat, dft_M1, dft_N1, fp,"deblur");
  cvWaitKey(-1);
}
...
void cvShowInvDFT(IplImage* im, CvMat* dft_A, int dft_M,int dft_N, FILE *fp,
    char* src) {
